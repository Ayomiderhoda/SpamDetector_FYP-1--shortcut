...
def dashboard():
    ...
    # Build daily counts for the trend chart (last 7 days seen in history)
    from collections import OrderedDict
    daily = OrderedDict()
    for record in h:
        day = record["date"].split(" ")[0]  # "DD/MM/YYYY"
        daily.setdefault(day, {"spam": 0, "ham": 0})
        if record["result"] == "Spam":
            daily[day]["spam"] += 1
        else:
            daily[day]["ham"] += 1

    days       = list(daily.keys())[-7:]  # last 7 days max
    spam_series = [daily[d]["spam"] for d in days]
    ham_series  = [daily[d]["ham"] for d in days]

    return render_template("dashboard.html",
        name=session["name"], total=len(h), spam=spam, safe=safe,
        chart_labels=days, chart_spam=spam_series, chart_ham=ham_series)
...
def history():
    if "email" not in session:
        return redirect(url_for("login"))
    raw = HISTORY.get(session["email"], [])
    records = list(reversed(raw))

    # Data for the "Spam Probability Over Time" line chart (chronological order)
    chart_labels = [r["date"] for r in raw]
    chart_probs  = [r["prob"] for r in raw]

    return render_template("history.html", name=session["name"], records=records,
                           chart_labels=chart_labels, chart_probs=chart_probs)
